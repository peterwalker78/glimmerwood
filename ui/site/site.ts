...
const motes = document.getElementById("motes") as HTMLElement;
const wake = document.getElementById("wake") as HTMLButtonElement;
const caughtSoFar = document.getElementById("caught") as HTMLElement;
const factText = document.getElementById("fact-text") as HTMLElement;
const title = document.getElementById("greeting-title") as HTMLElement;
const line = document.getElementById("greeting-line") as HTMLElement;
const factSource = document.getElementById("fact-source") as HTMLAnchorElement;
...
function choose(now: Date, from = 0): SitePlace[] {
...
function showFact(): void {
  ...
  if (FACTS.length > 1 && next === lastFact) next = (next + 1) % FACTS.length;
  lastFact = next;
...
function greet(words: { title: string; line: string }): void {
...
function wispAt(): { x: number; y: number } {
  const box = scene.getBoundingClientRect();
  const wisp = dozing.getBoundingClientRect();
  return {
